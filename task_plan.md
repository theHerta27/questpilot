...
- [x] 复核 `chaldea-data` 当前提交、许可证声明、文件来源与上游归属
- [x] 为 `dropData.json` 建立固定提交、`domusVer`、SHA-256、抓取时间与许可状态清单
- [x] 原始文件仅进入 gitignored 本地缓存；公开仓库只保留 Adapter、清单和合成 Fixture
- [x] 许可不明确时，UI 和文档明确标记“本地验证数据，不随项目再分发”
- **门禁：** 来源、哈希或许可状态缺失时禁止 publish
- **状态：** complete

### P2：小规模真实数据集发布（1–2 天）
- [x] 从 Atlas CN 建立关卡 ID、名称、章节、AP 与永久性事实
- [x] 从社区数据读取掉落率、样本数和数据集版本，不混入 Atlas Adapter
- [x] 只选择与 M1 演示目标有交集的 3–5 种材料、10–20 个永久自由关卡
- [x] 排除活动、随机敌人、缺少样本数、无法映射 Atlas 关卡 ID 的记录
- [x] 实现 `fetch → cache → adapt → validate → stage → publish` 与整批回滚
- [x] 页面/API 展示数据版本、样本量、候选范围、抓取时间与许可状态
- [x] 缓存经许可的角色与材料图片；下载失败使用本地占位图且不影响核心流程
- [x] 图片清单记录来源与本地缓存状态，不长期热链第三方服务器
- **门禁：** 损坏、空覆盖、低样本或 Atlas 映射失败的数据集不得发布
- **状态：** complete

### P3A：确定性规划基线（1 天）
- [x] 明确目标函数：固定候选集内最小期望 AP，次级目标为较少运行次数
- [x] 输入约束：材料缺口、当前 AP、苹果、每日时间、截止日期与单次耗时
- [x] 先实现简单、确定性、可端到端验证的基线规划器
- [x] 未覆盖材料返回“无已验证路线”，不得补猜掉落率

### P3B：有界 branch-and-bound（1–2 天）
- [x] 在基线可运行的前提下实现 branch-and-bound，并设置节点数和时间上限
- [x] 用穷举 Oracle 验证小型合成问题的局部最优性
- [x] 达到搜索上限时返回可验证的 best-so-far 与明确降级说明
- [x] 同一种求解实现连续失败两次后停止重写，保留基线规划器并记录原因
- **边界：** 只对固定候选集声明局部最优，不做全服全量或活动全局最优
- **状态：** complete

### P4：LangGraph 与 Harness 接入（1–2 天）
- [x] 使用既有流程：`calculate_gap → load_dataset → search_candidates → generate → validate`
- [x] Checkpoint 保存目标、库存、Atlas 版本、掉落数据版本与规划器版本
- [x] 执行预算、超时、幂等重试、循环检测与恢复
- [x] 确定性验证材料覆盖、库存、AP、日期、候选范围和数据版本
- [x] Trace 记录节点、工具参数摘要、候选淘汰原因、规划耗时与降级原因
- **约束：** 模型只解析目标与解释结果，不参与数值优化或掉落率猜测
- **状态：** complete

### P5：前端计划闭环（1–2 天）
- [x] 复用 M1 多目标清单，增加 AP、苹果、截止日期和每日时间输入
- [x] 增加自然语言入口，通过 DeepSeek V4 Flash 解析一个或多个角色/技能/当前等级/目标等级
- [x] 同名与低置信度候选要求用户选择；解析目标按“后一次完整覆盖”加入清单
- [x] 材料缺口与路线数值仍只由确定性工具计算，模型不得估算掉率或做数值优化
- [x] 展示简化模型解析结果、工具调用步骤与最终确定性计划
- [x] 展示关卡、次数、期望掉落、样本数、预计 AP 与资源是否足够
- [x] 固定展示版本、候选范围、“局部最优”与社区观测不确定性
- [x] 展示无覆盖、预算不足、截止日期不可达和搜索降级状态
- [x] 从计划结果进入 Trace，查看数据与验证证据
- [x] 搜索、目标、材料缺口、解析结果和路线尽量图文结合；所有图片提供 alt 与文字后备
- **状态：** complete

### P6：分层验收与暂停（1–2 天）
- [x] Adapter/版本/索引/过滤/回滚单元测试
- [x] 规划器穷举 Oracle、确定性、预算、日期、无覆盖与降级测试
- [x] FakeModel 规划轨迹与 Checkpoint 恢复测试
- [x] 10 条真实自然语言规划冒烟任务
- [x] 浏览器 E2E：自然语言/手工多目标 → 后输入覆盖 → 缺口 → 资源边界 → 局部路线 → Trace
- [x] 输出真实测试报告并暂停，等待是否进入 Mooncell 生产 RAG 或作品集收尾
- **状态：** complete / paused_for_review
